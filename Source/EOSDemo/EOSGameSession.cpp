...
void AEOSGameSession::BeginPlay()
{
    ...
    {
        //Create custom session in command line
        FString CommandLineMapName = "Default";
        FParse::Value(FCommandLine::Get(), *(SETTING_MAPNAME.ToString() + "="), CommandLineMapName);
        FString CommandLineGameMode = "Default";
        FParse::Value(FCommandLine::Get(), *(SETTING_GAMEMODE.ToString() + "="), CommandLineGameMode);

        CreateSession(CommandLineMapName, CommandLineGameMode); // Should parametrized Key/Value pair for custom attribute
    }
}
...
void AEOSGameSession::CreateSession(FString GameMap, FString GameMode) // Dedicated Server Only
{
    ...
    SessionSettings->Settings.Add(SETTING_GROUP, FOnlineSessionSetting(SETTING_GROUP_DEFAULT, EOnlineDataAdvertisementType::ViaOnlineService));
    SessionSettings->Settings.Add(SETTING_MAPNAME, FOnlineSessionSetting((GameMap), EOnlineDataAdvertisementType::ViaOnlineService));
    SessionSettings->Settings.Add(SETTING_GAMEMODE, FOnlineSessionSetting((GameMode), EOnlineDataAdvertisementType::ViaOnlineService));
